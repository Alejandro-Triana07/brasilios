...
export class ClientesService {
  static async crearCliente(input: CrearClienteInput): Promise<{ id: number }> {
    ...
    const [result] = await pool.query<ResultSetHeader>(
      ...
      [nombre, apellido, input.correo, hash]
    );
  ...
  static async listarClientes(filtro?: string): Promise<RowDataPacket[]> {
    const where = filtro
      ? `AND (
    ...
    const [rows] = await pool.query<RowDataPacket[]>(
      `SELECT u.id_usuario AS id, u.nombre, u.apellido, NULL AS telefono,
  ...
  static async obtenerClientePorId(id: number): Promise<RowDataPacket> {
    const [rows] = await pool.query<RowDataPacket[]>(
      `SELECT u.id_usuario AS id, u.nombre, u.apellido, NULL AS telefono,
              u.correo_electronico AS correo, (u.estado = 'activo') AS activo,
  ...
  static async actualizarCliente(id: number, input: ActualizarClienteInput): Promise<void> {
  ...
  static async eliminarCliente(id: number, adminId: number): Promise<void> {
  ...
  static async historialServiciosCliente(id: number): Promise<RowDataPacket[]> {
    await this.obtenerClientePorId(id);

    const [rows] = await pool.query<RowDataPacket[]>(
      `SELECT r.id_reserva AS cita_id, r.fecha, r.hora AS hora_inicio, r.estado,
  ...
  private static async validarDuplicados(correo?: string, telefono?: string, excluirId?: number): Promise<void> {
    if (correo) {
      const [rows] = await pool.query<RowDataPacket[]>(
        `SELECT id_usuario FROM usuario_rol WHERE correo_electronico = ? ${excluirId ? 'AND id_usuario <> ?' : ''} LIMIT 1`,
        excluirId ? [correo, excluirId] : [correo]
      );
      if (rows.length > 0) {
